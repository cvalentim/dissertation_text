...
y(x) = m*x + b
fit y(x) "random_fpair.s3.csv" via m, b

g(x) = m1*x + b1
fit g(x) "random_rmqNaive.s3.csv" via m1, b1

k(x) = m2*x + b2
fit k(x) "random_rmqSt.s3.csv" via m2, b2

plot "random_fpair.s3.csv" using 1:2 with points title 'Fpair', \
     "random_rmqNaive.s3.csv" using 1:2 with points title 'RmqBucket', \
     "random_rmqSt.s3.csv" using 1:2 with points  title 'RmqSt',\
